...
namespace {
    ...
    namespace tk {
        ...
        public:
            ...
            enum bd_type {
                first_deriv = 1, second_deriv = 2, not_a_knot = 3
            };
...
// -----------------------
        ...
        void spline::set_points(const std::vector<double> &x,
            ...
            assert(x.size() >= 3);
            // not-a-knot かつ x.size() == 3 の時の特別措置を追加したため、このassertを後ろにずらした
...
//    if (m_left == not_a_knot || m_right == not_a_knot) assert(x.size() >= 4);
            ...
            // 追加点: not-a-knot かつ x.size() == 3 の時の特別措置
            if (n == 3 && type == cspline && m_left == not_a_knot && m_right == not_a_knot) {
                m_d.resize(n + 1);  // x^3  m_d[n]にp0をこっそり仕込む
                m_c.resize(n);  // x^2
                m_b.resize(n);  // x^1
                const double p2 = ((y[0] - y[1]) * (x[0] - x[2]) - (y[0] - y[2]) * (x[0] - x[1])) /
                                  ((x[0] - x[1]) * (x[0] - x[2]) * (x[1] - x[2]));
                const double p1 = (y[0] - y[1]) / (x[0] - x[1]) - p2 * (x[0] + x[1]);
                const double p0 = y[0] - p2 * x[0] * x[0] - p1 * x[0];
                std::fill(m_d.begin(), m_d.end() - 1, 0);
                std::fill(m_c.begin(), m_c.end(), p2);
                std::fill(m_b.begin(), m_b.end(), p1);
                m_d[n] = p0;
        ...
        double spline::operator()(double x) const {
            ...
            // 変更点 (m_d[n]にx^0の項を仕込んである)
            if (n == 3 && m_type == cspline && m_left == not_a_knot && m_right == not_a_knot) {
                return ((m_d[0] * x + m_c[0]) * x + m_b[0]) * x + m_d[n];
            }
        ...
        std::vector<double> spline::solve(double y, bool ignore_extrapolation) const {
            ...
            for (size_t i = 0; i < n - 1; i++) {
                ...
                for (double r: root) {
                    ...
                    if ((-eps <= r) && (r < m_x[i + 1] - m_x[i])) {
                        double new_root = m_x[i] + r;
        ...
        namespace internal {
            ...
            std::vector<double> solve_quadratic(double a, double b, double c,
                ...
                for (double &x_v: x) {
                    for (int k = 0; k < newton_iter; k++) {
                        double f = (c * x_v + b) * x_v + a;
                        double f1 = 2.0 * c * x_v + b;
            ...
            std::vector<double> solve_cubic(double a, double b, double c, double d,
                ...
                for (double &z_v: z) {
                    ...
                    // improve solution via newton steps
                    for (int k = 0; k < newton_iter; k++) {
                        double f = ((z_v + c) * z_v + b) * z_v + a;
                        double f1 = (3.0 * z_v + 2.0 * c) * z_v + b;
                        // only adjust if slope is large enough
                        if (fabs(f1) > 1e-8) {
                            z_v -= f / f1;
                        }
